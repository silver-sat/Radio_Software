...
public:
    {
        CircularBuffer<char, PACKETSIZE> packet; // Unencoded packet data
        ...
        unsigned int size()
        {
            ...
            if (data.size() > 0)
            {
                ...
                for (index; (data[nextFEND] != FEND); index++)
                    packetsize++;
            ...
            void decapsulate()
            {
                // Trim excess bytes off
                for (unsigned int i = 0; i < index; i++)
                {
                    packet.shift()
                }
                for (unsigned int i = packet.size(); packet[i] != FEND; i--)
                {
                    // TODO: Return to this. I am not sure if I wrote it right
                }
                
                
                // Convert nextFEND to a constant for the following for loop
                const unsigned int NEXTFEND = nextFEND;
