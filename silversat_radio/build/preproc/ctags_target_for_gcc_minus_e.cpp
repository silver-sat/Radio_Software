# 1 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino"
/**
...
*/
# 58 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino"
...
// custom local files
# 76 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 77 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 78 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 79 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 80 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 81 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 82 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 83 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 84 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2

// the AX library
# 87 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2

# 89 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 90 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 91 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2


# 94 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 95 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 96 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
# 97 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino" 2
...
CircularBuffer<byte, 512 /* this buffer can be smaller because we control the rate at which packets come in*/> cmdbuffer;
CircularBuffer<byte, 8192 /* how many packets do we need to buffer at most during a TCP session?*/> databuffer;
CircularBuffer<byte, 512 /* at most 4 256-byte packets, but if storing Packet class objects, need to figure out how big they are*/> txbuffer; //txbuffer should only hold decoded kiss packets (255 bytes max), probably packet class objects
...
__attribute__((__aligned__(256))) static const uint8_t _dataoperating_frequency[(sizeof(int)+255)/256*256] = { }; FlashStorageClass<int> operating_frequency(_dataoperating_frequency);;
__attribute__((__aligned__(256))) static const uint8_t _dataclear_threshold[(sizeof(byte)+255)/256*256] = { }; FlashStorageClass<byte> clear_threshold(_dataclear_threshold);;
...
void setup()
{
    ...
    //reset indicator
    int state{0};
    Log.notice((reinterpret_cast<const __FlashStringHelper *>(("**********BOARD RESET*********\r\n"))));
    for (int i=0; i<10; i++)
        {
            digitalWrite((32u), state);
            state = !state;
    ...
    Log.notice((reinterpret_cast<const __FlashStringHelper *>(("starting up the radio\r\n"))));
    SPI.begin();
...
void loop()
{
# 266 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino"
    // interface handler - the interface handler processes packets coming in via the serial interfaces.
    ...
    while (Serial0.available() > 0)
    {
        ...
        if (cmdbuffer.isFull()) Log.error((reinterpret_cast<const __FlashStringHelper *>(("ERROR: CMD BUFFER OVERFLOW\r\n"))));
    }
    ...
    {
        if (Serial1.available() > 350) Log.error((reinterpret_cast<const __FlashStringHelper *>(("SERIAL BUFFER OVERFLOW"))));
        ...
        if (databuffer.isFull()) Log.error((reinterpret_cast<const __FlashStringHelper *>(("ERROR: DATA BUFFER OVERFLOW\r\n"))));
    }
    ...
    {
        Log.notice((reinterpret_cast<const __FlashStringHelper *>(("command received, processing\r\n"))));
        ...
        if (freemem < free_mem_minimum)
        {
            ...
            Log.notice((reinterpret_cast<const __FlashStringHelper *>(("min freememory updated: %d\r\n"))),freeMemory());
        }
    ...
    if (datapacketsize != 0)
    {
        ...
        if (txbuffer.size() == 0) // just doing the next packet to keep from this process from blocking too much
        {
            ...
            Log.trace((reinterpret_cast<const __FlashStringHelper *>(("unwrapped packet size: %i \r\n"))), datapacket.packetlength);
...
# 349 "C:\\GitHub\\Radio_Software\\silversat_radio\\silversat_radio.ino"
            ...
            if (radio.modulation.il2p_enabled == 1)
            {
                ...
                Log.trace((reinterpret_cast<const __FlashStringHelper *>(("construct IL2P packet\r\n"))));
                ...
                //scramble it
                Log.trace((reinterpret_cast<const __FlashStringHelper *>(("scrambling header\r\n"))));
                il2p_scramble_block(il2p_header_precoded, il2p_header_scrambled, 13);
                Log.verbose((reinterpret_cast<const __FlashStringHelper *>(("scrambled header: \r\n"))));
                for (int i = 0; i < 13; i++) Log.verbose("%X, ", il2p_header_scrambled[i]);
                Log.verbose("\r\n");
                //now encode it
                Log.trace((reinterpret_cast<const __FlashStringHelper *>(("encoding header\r\n"))));
                ...
                Log.verbose((reinterpret_cast<const __FlashStringHelper *>(("parity:\r\n"))));
            ...
            if (radio.modulation.il2p_enabled == 1)
            {
                Log.trace((reinterpret_cast<const __FlashStringHelper *>(("initial txbuffer size: %i\r\n"))), txbuffer.size());
                unsigned char il2p_framing[3]{0xF1, 0x5E, 0x48};

                //re-add the command byte
                Log.trace((reinterpret_cast<const __FlashStringHelper *>(("adding the command byte\r\n"))));
                txbuffer.push(datapacket.commandcode);

                //first the framing bytes
                Log.trace((reinterpret_cast<const __FlashStringHelper *>(("adding on the IL2P framing bytes\r\n"))));
                for (int i=0; i< 3; i++) txbuffer.push(il2p_framing[i]);
                datapacket.packetlength += 3; //add the three bytes to the total

                //next the header
                Log.trace((reinterpret_cast<const __FlashStringHelper *>(("pushing the IL2P header\r\n"))));
                for (int i = 0; i < 13; i++) txbuffer.push(il2p_header_scrambled[i]);

                //next the header parity
                Log.trace((reinterpret_cast<const __FlashStringHelper *>(("pushing the IL2P header parity\r\n"))));
                for (int i = 0; i < 2; i++) txbuffer.push(parity_header[i]);

                //next the data (scrambled)
                Log.trace((reinterpret_cast<const __FlashStringHelper *>(("pushing the IL2P data\r\n"))));
                ...
                Log.trace((reinterpret_cast<const __FlashStringHelper *>(("pushing the IL2P data parity\r\n"))));
    ...
    if (transmit == true)
    {
        ...
        if (datapacketsize == 0 && txbuffer.size() == 0) // datapacketsize should still be nonzero until the buffer is processed again (next loop)
        {
            ...
            radio.setReceive();
            Log.notice((reinterpret_cast<const __FlashStringHelper *>(("State changed to FULL_RX\r\n"))));
        }
        ...
        {
            Log.notice((reinterpret_cast<const __FlashStringHelper *>(("transmitting packet\r\n"))));
            Log.verbose((reinterpret_cast<const __FlashStringHelper *>(("datapacket.packetlength: %i\r\n"))), datapacket.packetlength);
            Log.verbose((reinterpret_cast<const __FlashStringHelper *>(("txbuffer.size: %i\r\n"))), txbuffer.size());
            ...
            Log.verbose((reinterpret_cast<const __FlashStringHelper *>(("databufflen (post transmit): %i\r\n"))), databuffer.size());
            Log.verbose((reinterpret_cast<const __FlashStringHelper *>(("cmdbufflen (post transmit): %i\r\n"))), cmdbuffer.size());
            Log.verbose((reinterpret_cast<const __FlashStringHelper *>(("datapacket.packetlength (post transmit): %i\r\n"))), txbuffer.size());
            Log.trace((reinterpret_cast<const __FlashStringHelper *>(("max S0 tx buffer load: %i\r\n"))), max_buffer_load_s0);
            Log.trace((reinterpret_cast<const __FlashStringHelper *>(("max S1 tx buffer load: %i\r\n"))), max_buffer_load_s1);
            Log.trace((reinterpret_cast<const __FlashStringHelper *>(("max databuffer load: %i\r\n"))), max_databuffer_load);
            Log.verbose((reinterpret_cast<const __FlashStringHelper *>(("max cmdbuffer load: %i\r\n"))), max_commandbuffer_load);
            Log.verbose((reinterpret_cast<const __FlashStringHelper *>(("max txbuffer load: %i\r\n"))), max_txbuffer_load);
            if (databuffer.size() > 4096)
            {
                Log.warning((reinterpret_cast<const __FlashStringHelper *>(("DATABUFFER at half full\r\n"))));
                Log.warning((reinterpret_cast<const __FlashStringHelper *>(("buffer size: %d\r\n"))), databuffer.size());
            }
            if (max_buffer_load_s0 > 350) Log.warning((reinterpret_cast<const __FlashStringHelper *>(("serial0 buffer overflow\r\n"))));
            if (max_buffer_load_s1 > 350) Log.warning((reinterpret_cast<const __FlashStringHelper *>(("serial1 buffer overflow\r\n"))));
            Log.trace((reinterpret_cast<const __FlashStringHelper *>(("freememory: %d\r\n"))),freeMemory());
        }
    }
    ...
    if (transmit == false)
    {
        if (radio.receive())
        {
            ...
            byte rxpacket[514];
            Log.trace((reinterpret_cast<const __FlashStringHelper *>(("got a packet!\r\n"))));
            Log.trace((reinterpret_cast<const __FlashStringHelper *>(("packet length: %i\r\n"))), radio.rx_pkt.length); // it looks like the two crc bytes are still being sent (or it's assumed they're there?)
            Log.trace((reinterpret_cast<const __FlashStringHelper *>(("freememory: %d\r\n"))),freeMemory());
            rxlooptimer = micros();
            ...
            if ((radio.modulation.framing & 0xE) == (2 << 1) || (radio.modulation.il2p_enabled)) command_offset=0;
            ...
            Log.trace((reinterpret_cast<const __FlashStringHelper *>(("kiss packet length: %d\r\n"))),rxpacketlength);
            Log.trace((reinterpret_cast<const __FlashStringHelper *>(("command byte: %X\r\n"))), radio.rx_pkt.data[command_offset]);
        ...
        { // the fifo is empty
            ...
            //if ((datapacketsize != 0) && channelclear == true )
            {
                ...
                Log.notice((reinterpret_cast<const __FlashStringHelper *>(("delay %lu\r\n"))), micros() - rxlooptimer); // for debug to see what actual delay is
                ...
                Log.notice ((reinterpret_cast<const __FlashStringHelper *>(("State changed to FULL_TX\r\n"))));
                transmit = true;
            }
        }
    }
