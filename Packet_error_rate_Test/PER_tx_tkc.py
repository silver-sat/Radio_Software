...
def main():
    tx_serial_port = serial.Serial('/dev/serial0', 19200) # Port to send on. Leave it at /dev/ttyACM0 for now
    #quantity = 3 # Leave it 1024 for now.

    packet_start = b'\xC0\x00'
    packet_finish = b'\xC0'
    inter_packet_delay = 0.25  # 200 mS between packets, allow 250 if RS is on.
    ...
    seq_num = 0
    while (1):  #go ahead an let it go on forever.  it will take a very long time to loop around
